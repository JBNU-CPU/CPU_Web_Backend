...
public class AdminController {
    ...
    @ApiResponse(responseCode = "200", description = "요청에 성공하였습니다.", content = @Content(mediaType = "application/json"))
    public List<StudyDTO> getAllStudies() {return adminService.getAllStudy();}
    ...
    public ResponseEntity<?> acceptStudy(@PathVariable Long id){
        StudyDTO updateStudyDTO =  adminService.acceptStudy(id);
        return ResponseEntity.ok(updateStudyDTO);
    }
    ...
    public ResponseEntity<?> unacceptStudy(@PathVariable Long id){
        StudyDTO updateStudyDTO =  adminService.unacceptStudy(id);
        return ResponseEntity.ok(updateStudyDTO);
    }
